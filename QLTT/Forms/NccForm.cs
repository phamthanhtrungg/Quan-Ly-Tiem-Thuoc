...
;
        ...
        private void dataGridView1_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.ColumnIndex == 4)
            {
                ...
                var maNccToDelete = currentRow.Cells[0].Value.ToString().Trim();
                if (MessageBox.Show("Bạn có muốn xóa bản ghi này không?", "", MessageBoxButtons.YesNo) == DialogResult.Yes)
                {
                    if (nccLogic.XoaNCC(maNccToDelete))
                    {
